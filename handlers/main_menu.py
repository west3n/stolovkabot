...
async def main_menu_msg(msg: types.Message):
    await msg.answer("Выберите один из вариантов:", reply_markup=await inline.main_menu())


async def main_menu_call(call: types.CallbackQuery):
    try:
        await call.message.edit_text("Выберите один из вариантов:", reply_markup=await inline.main_menu())
